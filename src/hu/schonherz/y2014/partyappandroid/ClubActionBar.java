...
public class ClubActionBar implements OnClickListener, OnMenuItemClickListener {
	...
	case 7:
		...
		public void onClick(DialogInterface dialog, int which) {
			...
			Toast t = Toast.makeText(activity, "Nem adtad meg hány napra szeretnéd a kiemelést.",
				Toast.LENGTH_LONG);
			t.show();
			return;
		    }
		    ...
		    if (days <= 0) {
			Toast t = Toast.makeText(activity, "Nem kérhetsz nullánál kisebb napnyi kiemelést!",
				Toast.LENGTH_LONG);
			t.show();
			return;
		    }

		    Session.getSearchViewClubs().get(clubListPosition).highlite_expire = Session.getInstance()
			    .getActualCommunicationInterface().setHighlightExpire(clubId, days);

		}
	    });
	    builder.setPositiveButton("Mégse", new android.content.DialogInterface.OnClickListener() {
