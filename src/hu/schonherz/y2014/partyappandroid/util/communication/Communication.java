...
public class Communication implements CommunicationInterface {
    ...
    public List<Club> getFavoriteClubsFromUserId(int user_id) {
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                ret.add(new Club(jsonObject.getInt("id"), jsonObject.getString("name"),
                        jsonObject.getString("address"), 1, jsonObject.getString("highlight_expire")));
            }
            return ret;
        } catch (Exception e) {
    ...
    public List<Club> searchClubs(String name, String cityname, String type, List<String> selectedServices, int offset,
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                JSONObject jsonObject = jsonArray.getJSONObject(i);
                ret.add(new Club(jsonObject.getInt("id"), jsonObject.getString("name"),
                        jsonObject.getString("address"), jsonObject.getString("highlight_expire")));
            }
            return ret;
        } catch (Exception e) {
    ...
    public List<Rating> getRatings(int clubId) {
        ...
        try {
            ...
            for (int i = 0; i < ja.length(); i++) {
                JSONObject jo = ja.getJSONObject(i);
                ret.add(new Rating(jo.getInt("user_id"), jo.getString("name"), (float) jo.getDouble("value"), jo
                        .getString("comment"), jo.getInt("approved")));
            }
    ...
    public List<AdminRating> getNotApprovedRatings() {
        ...
        try {
            ...
            for (int i = 0; i < ja.length(); i++) {
                JSONObject jo = ja.getJSONObject(i);
                ret.add(new AdminRating(jo.getInt("user_id"), jo.getString("name"), (float) jo.getDouble("value"), jo
                        .getString("comment"), jo.getInt("approved"), jo.getInt("club_id"), jo.getString("clubName")));
            }
