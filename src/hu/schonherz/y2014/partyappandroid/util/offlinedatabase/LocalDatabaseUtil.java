...
public class LocalDatabaseUtil {
	...
	public User loginWithNickName(String nickName, String password) {
		String selectQuery = String.format(
				"SELECT * FROM user WHERE nick_name = %s AND password = %s",
				nickName, password);
    ...
    public void insertUser(int id,String nickName,String password,String email,int sex,String birthday,int type) {
    	String guery=String.format("INSERT INTO user VALUES('%d','%s','%s','%s','%d',date('%s'),'%d',datetime('now'))",
			id,nickName,password,email,sex,birthday,type);
    	database.execSQL(guery);
    }
  ...
  //ok
	...
	public ArrayList<Club> userOwnClubs(int id) {
		...
		String selectQuery = String.format("SELECT club.* FROM user INNER JOIN"
				...
				+ "BY club.highlight_expire DESC, club.name",
				Integer.toString(id));
	...
	public Club searchFavoriteClub(int id, String adress, String type,
			String service) {

		String selectQuery = String
				.format("SELECT DISTINCT club.* FROM user INNER JOIN "
	...
	public Club searchOwnClub(int id, String adress, String type, String service) {
		...
		String selectQuery = String
				.format("SELECT DISTINCT club.* FROM user INNER JOIN owner "
						...
						+ "DESC, club.name", Integer.toString(id), adress,
						type, service);
    ...
    public void insertClub(int id, String name, String type, String description, String adress, String phonenumber,
		...
		String guery=String.format("INSERT INTO club VALUES('%d','%s','%s','%s','%s','%s','%s',date('%s'))",
				id,name,type,description,adress,phonenumber,email,highlight_expire);
	    	database.execSQL(guery);
    ...
    public void updateClub(int id, String name, String type, String description, String address, String phonenumber,
    	...
    	String guery=String.format("UPDATE club SET name='%s',type='%s',description='%s'," +
			"address='%s',phonenumber='%s',email='%s',highlight_expire=date('%s') WHERE id='%d'",name,
			type,description,address,phonenumber,email,highlite_expire,id);
    ...
    public Club clubInformation(int id) {
		...
		Club club = new Club(cursor.getInt(0), cursor.getString(1), cursor.getString(2),
				cursor.getString(3), cursor.getString(4), cursor.getString(5), cursor.getString(6),
				cursor.getString(7),0);    //erre odafigyelni, 0-ra allitja az approve-ot	
	...
	public Event eventInformationFromEventId(int event_id) {
		...
		String selectQuery = String.format(
				"SELECT * FROM event WHERE id = %d;",
				Integer.toString(event_id));

		Cursor cursor = database.rawQuery(selectQuery, null);

		out = new Event(event_id, cursor.getString(2), cursor.getString(3),
	...
	// új esemény
	public void insertNewEvent(int id, int club_id, String name,
			String description, String start_date, String music_type) {
	...
	// esemény frissítése
	public void updateEvent(int id, int club_id, String name,
			String description, String start_date, String music_type) {
		ContentValues values = new ContentValues();
		...
		database.update("event", values, "id = ?",
	...
	public void updateFavoriteClub(int id, int user_id, int club_id) {
		...
		database.update("favorite", values, "id = ?",
				new String[] { Integer.toString(id) });
	...
	public void updateOwnClub(int user_id, int club_id, int approved) {
		...
		database.update(
				...
				values,
				"club_id = ? AND user_id = ?",
				new String[] { Integer.toString(club_id),
	...
	public void deleteOwnClub(int user_id, int club_id, int approved) {
		...
		database.delete("owner", "club_id = ? AND user_id = ?", new String[] {
				Integer.toString(club_id), Integer.toString(user_id) });
	...
	public ArrayList<Event> favoritesEventInformation(int user_id) {
		...
		if (cursor.moveToFirst()) {
			do {
				Event actual_event = new Event(Integer.parseInt(cursor
    ...
    public ArrayList<MenuItem> getMenuItems(int club_id) {
		...
		if (cursor.moveToFirst()) {
		    do {
			MenuItem actual_menuItem = new MenuItem(cursor.getInt(0), cursor.getString(2),
				cursor.getInt(3), cursor.getString(7), cursor.getString(8),
	...
	public ArrayList<MenuItem> menuItemsPrice(int club_id) {
		...
		if (cursor.moveToFirst()) {
			do {
				MenuItem actual_menuItem = new MenuItem(Integer.parseInt(cursor
						.getString(0)), cursor.getString(2),
	...
	public void insertNewMenuItemsPrice(int id, int club_id, String name,
			int price, int discount, String menu_category, int menu_sort,
	...
	public void updateMenuItemsPrice(int id, int club_id, String name,
			int price, int discount, String menu_category, int menu_sort,
			String currency, String unit) {
		ContentValues values = new ContentValues();
		...
		database.update("menu_item", values, "id = ?",
				new String[] { Integer.toString(id) });
	}
