...
public class RegisterActivity extends ActionBarActivity implements
	...
	public void onClickHandler(View v) {
		if (v.getId() == R.id.register_button_register) {
			...
			final String userBirthday = dateOfBirthEditText.getText()
					.toString();
			...
			Session.getInstance().progressDialog = ProgressDialog.show(this,
					"Kérlek várj", "Regisztráció folyamatban...", true, false);
			new Thread(new Runnable() {

				@Override
				public void run() {
					final User newUser = Session
							.getInstance()
							.getActualCommunicationInterface()
							.registerANewUser(userName, userPassword,
									userEmail, userSexInt, userBirthday);
					RegisterActivity.this.runOnUiThread(new Runnable() {
						...
						public void run() {
							...
							Intent newIntent = new Intent(
									RegisterActivity.this, ClubsActivity.class);
							Session.getInstance().dismissProgressDialog();
							if (LoginActivity.instance != null) {
								LoginActivity.instance.finish();
							}
							new DoneToast(RegisterActivity.this,
									"Sikeres regisztráció!").show();
